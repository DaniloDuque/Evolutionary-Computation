...
(provide _+_ _-_ _*_ _lg_ _/_ entry eval-exp evaluate cross-over make-gen mutation entry size eval-exp seed)
(require "input.scm" "mutation.scm" "operators.scm")
...
(define (eval-exp fun x y)
    (cond
        ...
        (#t ((car fun) (eval-exp (cadr fun) x y) (eval-exp (caddr fun) x y)))
    )
)
